 ...
 *  - 分段器按双阈值+语义边界/块切换信号切段；切段是否入 state 由 inSplice 门控
 *  - state 级哈希去重；finish/异常均 flush 并 end
 *  - M3：肥段入精炼队列（fire-and-forget）；主流 error/abort 时按会话取消
...
export function installDetect(
  ...
  ctx.on('llm/stream', (reqOptions, next) => {
    ...
    const segmenter = new Segmenter(
      { segmentMinTokens: opts.segmentMinTokens, segmentMaxTokens: opts.segmentMaxTokens },
      (text: string, tokens: number) => {
        // 阈值前不产出总结；state 级去重防重试/重放
        if (!detector.inSplice) return
        ...
        try {
          for await (const chunk of inner) {
            ...
            if (t === 'block-start') {
              ...
              state.thinkingTokens = detector.thinkingTokens
              state.updatedAt = Date.now()
            } else if (t === 'finish') {
              segmenter.flush()
              store.end(key)
