...
- **双阈值**：
  ...
  - `segmentMaxTokens = 3000`：硬上限，到点强制切，保证缓冲有界。
- **语义边界信号**（在最小窗口之上等待）：
  - 小节标题（Markdown 标题行）与要点行；
  - **行首**结构词：`其次 / 接下来 / 然后 / Finally / Second / Step N`（锚定行首，避免句中高频词误触发）；
  - 代码围栏闭合（`m` 标志锚定行尾）；
...
#### 4.3.2 小模型精炼（按需、可关）
- **触发门控**（省 token 核心）：段 token ≥ `refineMinSegmentTokens`（默认 1200，"够肥"才精炼）**且**整链已被判定为长思考 **且** 全局精炼开关开。
...
- **输入输出硬限制**：只喂段尾部 `min(段, 1500)` token；提示词为固定短模板；答案展示时截断到 ~60 token（240 字符）。
- **并发与取消**：独立队列，并发上限 1；**绝不 await 在主流迭代内**（fire-and-forget）；主流 error/abort 时按会话取消未完成精炼；正常 finish 让队列自然排空（精炼调用本身很短，~1.5s）。
