...
> 总结卡片不显示任何代码块/表格痕迹，计入总 token）；保留时按之前实现
> （原子+结构化摘要+可选精炼）；精炼输入裁剪三档可配置；段头同时显示原始输出
> token 与精炼实际 token。
...
1. **代码块/表格三态处理**（`codeBlockMode`/`tableMode`，默认 `ignore`）：
   - `ignore`（默认）：内容**不写进缓冲**（不占内存、不计段 token、不精炼），总结卡片**不显示**任何代码块/表格痕迹（不产出元信息段）；检测器仍计入总 token（阈值/进度）
...
| 键 | 默认 | 说明 |
|---|---|---|
| `codeBlockMode` | 'ignore' | 代码块处理：忽略（不写内存、卡片不显示）/ 保留+跳过精炼 / 保留并精炼 |
