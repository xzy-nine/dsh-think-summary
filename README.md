...
- **长思考检测**：`llm/stream` 瀑布实时观察 `reasoning-delta`，轻量 token 计数
  ...
  旁路流（子代理 / 标题生成）自动过滤。
- **语义分段**：双阈值（最小窗口 1500 / 硬上限 3000 token）+ 语义边界信号
  （行首结构词、标题、要点、代码围栏、`blockType` 切换强信号），只缓存未总结余量，
  内存有界、state 级去重。
- **逐段总结**：启发式提取（0 token）即时出；**开启精炼后全部分段**都会用会话
  provider 的最小模型精炼摘要（输入截断段尾部 1500 token、展示截断 ~60 token、
  并发 1、异常自动回退启发式、主流异常时按会话取消）。
- **实时展示**：聊天区 composer 上方实时面板（思考进度 + 滚动分段摘要），客户端
  轮询 `/api/think-summary/state`；总结**不写回会话上下文**，零污染。
...
| 段硬上限 | `3000` | 到点强制切，保证缓冲有界 |
| 精炼 | `true` | 开启后**所有分段**都用会话 provider 最小模型精炼摘要 |
